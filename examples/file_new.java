...
public class InteractionEvent {
	...
	/**
	 * The set of intern'ed strings. So String.intern() doesn't need to be called multiple times.
	 */
	private static final HashMap<String, String> INTERNSTRING = new HashMap<String, String>();
	...
	public InteractionEvent(Kind kind, String structureKind, String handle, String originId, String navigatedRelation,
		...
		this.kind = kind;
		this.structureKind = (structureKind != null) ? intern(structureKind) : null;
		this.structureHandle = (handle != null) ? intern(handle) : null;
		this.originId = intern(originId);
		this.navigation = (navigatedRelation != null) ? intern(navigatedRelation) : null;
		this.delta = (delta != null) ? intern(delta) : null;
	...
	/**
	 * private intern to avoid the slow String.intern() call in the creation of the object
	 * 
	 * @param s
	 *            The string to be intern'ed
	 * @return the intern'ed string
	 */
	private static String intern(String s) {

		if (!INTERNSTRING.containsKey(s)) {
			s = s.intern();
			INTERNSTRING.put(s, s);
		} else {
			s = INTERNSTRING.get(s);
		}
		return s;
	}
